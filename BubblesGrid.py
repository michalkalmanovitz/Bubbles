...
def find_bubble_location_in_grid(bullet_bubble):
    ...
    for row in range(get_length()):
        if fitted_row == -1:
            tmp_bubble = bubbles_grid[row][0]
            dist = abs(bullet_bubble["center_y"] - tmp_bubble["center_y"])
            if 0 <= dist <= consts.BUBBLE_RADIUS:
                fitted_row = row

    for column in range(len(bubbles_grid[0])):
        if fitted_column == -1:
            tmp_bubble = bubbles_grid[fitted_row][column]
            dist = abs(bullet_bubble["center_x"] - tmp_bubble["center_x"])
            if 0 <= dist <= consts.BUBBLE_RADIUS:
